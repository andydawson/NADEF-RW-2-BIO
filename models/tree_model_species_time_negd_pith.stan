data {
  ...
  int<lower=0> pith2tree[N_pith];
  int<lower=0> pith2year[N_pith];
...
transformed parameters {
  ...
  matrix[N_trees, N_years] d_latent;
...
model {
  ...
  for (i in 1:N_dbh) {
    ...
    d[i] ~ student_t(3.0, d_latent[d2tree[i],d2year[i]], exp(sig_d_obs));
  }

  for (i in 1:N_pith) {
    //  print(pith2tree[i]);
//print(pith2year[i]);
    //0 ~ student_t(3.0, d_latent[pith2tree[i], pith2year[i]], sig_pith_obs);
    //0 ~ student_t(3.0, d_latent[pith2tree[i], pith2year[i]], sig_pith_obs);
    0 ~ normal(d_latent[pith2tree[i], pith2year[i]], sig_pith_obs);
  }
}
