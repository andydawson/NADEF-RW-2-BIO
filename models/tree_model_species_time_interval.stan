...
parameters {
  ...
  real<lower=0,upper=30> d_init[N_trees];
...
transformed parameters {
  ...
  for (tree in 1:N_trees){
    ...
    d_latent[tree,rw_year_start[tree]] = d_init[tree] + 2 * x[tree, rw_year_start[tree]] / 10 ;
    for (year in (rw_year_start[tree]+1):rw_year_end[tree]) {
      d_latent[tree,year] = d_latent[tree, year-1] + 2 * x[tree, year] / 10 ;
    }
  }
...
model {
  ...
  // latent ring-width process model 
  for (tree in 1:N_trees) {
    for (year in rw_year_start[tree]:rw_year_end[tree]) {
      // if (logy[tree,year] == -999){
      // } else {
        x[tree, year] ~ lognormal(beta[tree] + beta_t[year, core2species[tree]], sig_x);
	//}
