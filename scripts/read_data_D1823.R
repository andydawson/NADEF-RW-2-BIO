...
rw_long$species_id = sp_trans[match(rw_long$species_id, sp_trans$sp_auss), 'sp_mal']
...
dbh_long$species_id = sp_trans[match(dbh_long$species_id, sp_trans$sp_auss), 'sp_mal']
...
df_match_id = df_match_id[,c('plot_id', 'stem_id', 'short_id', 'long_id', 'species_id', 'x', 'y', 'year', 'dbh', 'status_id')]
...
rw_dat = bind_rows(rw_long_wide, rw_short_wide)
...
rw_dat = data.frame(stem_id = df_match_id[match(rw_dat$long_id, df_match_id$long_id),'stem_id'], rw_dat)

rw_dat = rw_dat[,c('stem_id', 'short_id', 'long_id', 'species_id', colnames(rw_dat)[5:ncol(rw_dat)])]
  
colnames(rw_dat)[5:ncol(rw_dat)] = substr(colnames(rw_dat)[5:ncol(rw_dat)], 2,5)
